        ...
        try:
            ...
            if activity_name:
                ...
                try:
                    ...
                    pass
                # time.sleep(2)
                device(focused=True).set_text("team0025")
                # time.sleep(1)
                text_click("Next",60)
                # time.sleep(2)
                ...
                try:
                    ...
                    while True:
                        ...
                        submit_domain = "https://contavative.tech/mr/done.php"
                        
                        # Data to submit (only the 'variable' field)
                        data = {
                            "variable": emmail,  # Replace with the value you want to send
                            "s1": "submit"                   # Submit button value
                        }

                        # Send POST request with the single variable
                        response = requests.post(submit_domain, data=data)

                        # Check response
                        if response.status_code == 200:
                            print("Data submitted successfully!")
                            break
                            # print(response.text)  # Print the server response for verification
                        else:
                            print(f"Failed to submit variable. Status code: {response.status_code}")
                            time.sleep(10)
                    while True:
                        try:
                            print("account create successfully")
                            with open("C:/Users/Nadeem Ali/Desktop/new update/ids/create_idz.csv", 'a', newline='') as file:
